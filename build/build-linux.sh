...
#   pezmax-one-VERSION-linux-amd64.deb        pezmax-one-VERSION-linux-x86_64.pkg.tar.zst
#   pezmax-one-VERSION-linux-arm64.deb        pezmax-one-VERSION-linux-aarch64.pkg.tar.zst
#
# 安装后系统内路径：
...
build_deb() {
  ...
  # 客户端 auto-update pick_asset() 按 amd64/arm64 匹配 .deb 名，且优先带 linux 段
  local out="$DIST_DIR/${PKG_NAME}-${VERSION}-linux-${deb_arch}.deb"
...
build_arch_pkg() {
  ...
  # 客户端 auto-update pick_asset() 按 x86_64/aarch64 匹配 .pkg.tar.zst 名，且优先带 linux 段
  # 注意去掉了 pkgrel "-1"，保持与 .deb / .msi / .dmg 命名一致（<pkg>-<version>-<os>-<arch>.<ext>）
  local out="$DIST_DIR/${PKG_NAME}-${VERSION}-linux-${pacman_arch}.pkg.tar.zst"
  local workdir; workdir=$(mktemp -d)
